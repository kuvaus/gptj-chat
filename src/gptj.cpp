...
////////////                       API                        ////////////
//////////////////////////////////////////////////////////////////////////
struct GPTJModelContext {
    gpt_vocab vocab;
    gptj_model model;
};

GPTJModelContext* load_model(const char* filename) {
...
void free_model(GPTJModelContext* ctx) {
    ggml_free(ctx->model.ctx);
    delete ctx;
}

bool generate(GPTJModelContext* model_ctx, const char* prompt, GPTJParams params, char* response) {
    if (params.seed < 0) params.seed = time(NULL);
    if (params.n_threads <= 0) params.n_threads = std::min(4, (int32_t)std::thread::hardware_concurrency());
    ...
    std::string result;
    auto future = std::async(std::launch::async, display_frames);
    for (int i = embd.size(); i < embd_inp.size() + params.n_predict; i++) {
        if (embd.size() > 0 && !gptj_eval(model, params.n_threads, n_past, embd, logits, mem_per_token)) {
        ...
        if (i >= embd_inp.size()) {
            gpt_vocab::id id = gpt_sample_top_k_top_p(vocab, logits.data() + (logits.size() - model.hparams.n_vocab), params.top_k, params.top_p, params.temp, rng);
            embd.push_back(id);
        } else {
